...
function _dvDismissAuthAndInit() {
  ...
  // Refresh Multi-Tenant status banner if module is loaded
  if (typeof _mtRefreshStatusBanner === 'function') setTimeout(_mtRefreshStatusBanner, 500);
  if (typeof mtUpdateUIAfterLogin    === 'function') setTimeout(mtUpdateUIAfterLogin, 600);
}
...
function dvGenerateSyncCode() {
  ...
  // Also register as MT invite code if MT is active
  if (mtCfg?.spreadsheetId) {
